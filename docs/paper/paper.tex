...
We evaluate \textsc{NeuromemoryAI} on our benchmark suite, focusing on the unique capabilities that cognitive memory dynamics provide.

\paragraph{Multi-Session Continuity}
In a simulation of 10 sessions over 7 days, user preferences stated in session 1 were retrieved with Recall@3 of 40\% using pure FTS5 keyword matching. When augmented with an LLM for query expansion (converting ``user name'' to ``user name Alice Chen''), Recall@3 improved to 80\%. This highlights the system's design: \emph{memory dynamics} handle activation and retrieval ordering, while \emph{LLMs} handle semantic interpretation.

\paragraph{Hebbian Emergence and Spreading Activation}
The most compelling result: Hebbian links form automatically and \emph{improve retrieval} through spreading activation. We demonstrate this with a simple experiment:

\begin{enumerate}
    \item Add 3 ML-related memories (neural networks, gradient descent, PyTorch)
    \item Query ``Neural networks'' → returns 1 result (direct match only)
    \item Simulate usage: 5 queries that co-activate all 3 memories together
    \item Query ``Neural networks'' again → returns \textbf{3 results}
\end{enumerate}

The additional 2 memories were retrieved via Hebbian spreading activation, even though they don't contain ``Neural networks.'' This demonstrates emergent associative retrieval without NER or manual linking.

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\toprule
Condition & Results for ``Neural networks'' \\
\midrule
Before co-activation & 1 \\
After co-activation (no expansion) & 1 \\
After co-activation (with Hebbian expansion) & \textbf{3} \\
\bottomrule
\end{tabular}
\caption{Hebbian spreading activation improves recall}
\end{table}

\paragraph{Performance}
Zero-dependency implementation achieves competitive latency:
...
Operation & Latency \\
\midrule
Recall (avg, 100 memories) & 3.32ms \\
Recall (p99) & 4.14ms \\
Consolidation (100 memories) & 16.28ms \\
\bottomrule
\end{tabular}
\caption{Latency benchmarks on M1 Mac}
\end{table}

These latencies compare favorably to embedding-based systems, which require either API calls (50-200ms) or local embedding computation (10-50ms per query).

\paragraph{Forgetting and Memory Management}
Over 100 simulated days (10 memories/day), NeuromemoryAI's forgetting mechanism archives 64\% of low-value memories while preserving important ones:

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\toprule
System & Active Memories & Archived \\
\midrule
NeuromemoryAI & 359 & 641 (64\%) \\
Mem0/Zep (no forgetting) & 1000 & 0 \\
\bottomrule
\end{tabular}
\caption{Memory management after 100 days}
\end{table}

This improves retrieval signal-to-noise ratio: queries search 359 relevant memories instead of 1000 including noise.

\paragraph{Comparison with Vector-Based Systems}
Direct comparison with embedding-based memory (simulated Mem0/Zep latencies):
...
\midrule
Recall latency & 3.6ms & $\sim$60ms \\
External dependencies & None & OpenAI, Vector DB \\
Works offline & Yes & No \\
Automatic forgetting & Yes & No \\
Association learning & Hebbian (usage) & NER or manual \\
\bottomrule
\end{tabular}
\caption{System comparison}
\end{table}

\paragraph{Running the Benchmarks}
We release our benchmark suite for reproducibility:

\begin{verbatim}
cd neuromemory-ai
python benchmarks/run_benchmark.py --all
python benchmarks/run_benchmark.py --task hebbian
\end{verbatim}
